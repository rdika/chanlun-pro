...
class ExchangeTDX(Exchange):
    ...
    def support_frequencys(self):
        return {
            ...
            '15m': '15m', '10m': '10m', '5m': '5m', '2m': '2m', '1m': '1m'
        }
    ...
    def plate_stocks(self, code: str):
        ...
        if self.tdx_bkgn.tdx_path is not None:
            ...
            return [self.stock_info(_c) for _c in codes if self.stock_info(_c) is not None]
    ...
    def xdxr(self, market: int, project_code: str, code: str):
        ...
        if xdxr_file.is_file() is False or fun.timeint_to_str(int(xdxr_file.stat().st_mtime), '%Y-%m-%d') != now_day:
            need_update = True
        if need_update:
            client = TdxHq_API(raise_exception=True, auto_retry=True)
            with client.connect(self.connect_ip['ip'], self.connect_ip['port']):
