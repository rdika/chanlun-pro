...
def add_code_marks(
        code: str, _time: int, color: str, label: str, tooltip: str, shape: str = 'circle', show_freq: str = ''
) -> bool:
    """
    添加代码在 tv 时间轴显示的信息
    ...
    """
    mark = {
        'code': code, 'time': _time, 'color': color, 'label': label,
        'tooltip': tooltip, 'shape': shape, 'show_freq': show_freq
    }
    key = hashlib.md5(f"{mark['time']}_{mark['label']}_{mark['shape']}".encode('utf-8')).hexdigest()
